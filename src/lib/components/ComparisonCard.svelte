...
<div class="text-center">
  <div class="relative inline-block">
    ...
    {#if data.accented}
      <div class="absolute -top-2 -right-2 bg-accent-yellow text-navy-600 text-[10px] font-bold px-2 py-0.5 rounded-full">✓</div>
    {/if}
  </div>
  <h5 class="text-sm font-semibold mt-4 {data.accented ? 'text-accent-yellow' : 'text-white/60'}">{data.title}</h5>
