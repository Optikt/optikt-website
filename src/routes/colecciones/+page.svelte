<script lang="ts">
  ...
  let allGenders = $derived([...new Set(products.map(p => p.gender).filter(Boolean))] as string[]);
  let allMaterials = $derived([...new Set(products.map(p => p.material).filter(Boolean))] as string[]);
  ...
  let filtered = $derived(
    products
      .filter(p => {
        ...
        if (selectedMaterials.size > 0 && (!p.material || !selectedMaterials.has(p.material))) return false;
        return true;
      })
      .sort((a, b) => {
  ...
  let hasFilters = $derived(selectedCategories.size > 0 || selectedGender !== null || selectedMaterials.size > 0);
</script>
...
<section class="bg-navy-600 relative overflow-hidden">
  <div class="max-w-7xl mx-auto px-6 md:px-12 py-20 md:py-28 relative z-10">
    ...
    <h1 class="text-4xl md:text-5xl font-bold text-white mt-4 tracking-tight">Nuestras Colecciones</h1>
    <p class="text-white/60 mt-4 max-w-xl leading-relaxed">
      Explora nuestra selección de monturas y lentes diseñadas para realzar tu estilo y cuidar tu visión.
    </p>
  </div>
  <div class="absolute top-0 right-0 w-96 h-96 bg-accent-yellow/5 rounded-full blur-3xl pointer-events-none"></div>
</section>
      ...
      <aside class="lg:col-span-1 {sidebarOpen ? 'block' : 'hidden'} lg:block">
        ...
        >
          <div class="flex items-center justify-between">
            ...
            {#if hasFilters}
              <button onclick={resetFilters} class="text-xs text-accent-yellow font-semibold hover:underline cursor-pointer">
                Limpiar
              </button>
            {/if}
          </div>
          ...
          <div>
            <h3 class="text-xs font-bold text-navy-400 uppercase tracking-wider mb-3">Categoría</h3>
            <div class="space-y-2">
              {#each allTypes as type (type)}
                <label class="flex items-center gap-3 cursor-pointer group">
                  ...
                  />
                  <span class="text-sm text-navy-400 group-hover:text-navy-600 transition-colors">{typeLabels[type] ?? type}</span>
                </label>
              {/each}
            </div>
          </div>
          ...
          <div class="border-t border-stone-100 pt-6">
            ...
            <div class="flex flex-wrap gap-2">
              <button
                ...
                class="px-3 py-1.5 rounded-full text-xs font-semibold transition-all cursor-pointer {selectedGender === null
              ...
              {#each allGenders as g (g)}
                <button
                  onclick={() => setGender(g)}
                  class="px-3 py-1.5 rounded-full text-xs font-semibold transition-all cursor-pointer {selectedGender === g
                    ? 'bg-navy-600 text-white'
                    : 'bg-stone-100 text-navy-400 hover:bg-stone-200'}"
                >
                  {g}
          ...
          <div class="border-t border-stone-100 pt-6">
            <h3 class="text-xs font-bold text-navy-400 uppercase tracking-wider mb-3">Material</h3>
            <div class="space-y-2">
              {#each allMaterials as mat (mat)}
                <label class="flex items-center gap-3 cursor-pointer group">
                  <input
                    ...
                    class="appearance-none w-4 h-4 rounded border-2 border-stone-200 checked:bg-accent-yellow checked:border-accent-yellow transition-all duration-200 cursor-pointer"
                  />
                  <span class="text-sm text-navy-400 group-hover:text-navy-600 transition-colors">{mat}</span>
                </label>
              {/each}
            </div>
          </div>
      ...
      <div class="lg:col-span-3 space-y-8">
        <div use:fadeIn={{}} class="flex items-center justify-between">
          <p class="text-navy-400 text-sm font-medium hidden lg:block">
            Mostrando {paginated.length} de {filtered.length} producto{filtered.length !== 1 ? 's' : ''}
          </p>
            ...
            <div class="flex items-center justify-center gap-2 pt-4">
              ...
              {#each Array.from({ length: totalPages }, (_, i) => i + 1) as pg (pg)}
                {#if pg === 1 || pg === totalPages || Math.abs(pg - page) <= 2}
                  <button
                    onclick={() => { page = pg; }}
                    class="w-10 h-10 rounded-full text-sm font-semibold transition-all cursor-pointer {pg === page
                      ? 'bg-navy-600 text-white'
