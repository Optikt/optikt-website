<script lang="ts">
  ...
  const circleClasses = 'bg-white/5 border border-white/10 text-white/60 hover:bg-accent-yellow hover:text-navy-600 hover:border-accent-yellow';
...
<div class="flex gap-3" class:mt-6={variant === 'text'}>
  <a href={business.socialLinks[0].href} target="_blank" rel="noreferrer" aria-label="Instagram" class={linkClass}>
    <Instagram class="w-4 h-4" />
  </a>
  <a href={business.socialLinks[1].href} target="_blank" rel="noreferrer" aria-label="Facebook" class={linkClass}>
  ...
  {#if variant === 'circles'}
    <a href={business.socialLinks[2].href} target="_blank" rel="noreferrer" aria-label="WhatsApp" class="w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 bg-white/5 border border-white/10 text-white/60 hover:bg-green-500 hover:text-white hover:border-green-500">
