...
<section class="py-24 md:py-28 bg-navy-600 relative overflow-hidden">
  <div class="absolute inset-0 opacity-10 pointer-events-none">
    ...
    <div class="absolute bottom-0 right-1/3 w-72 h-72 bg-accent-blue rounded-full blur-[100px]"></div>
  </div>
  ...
  <div class="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
    ...
    <Marquee gap={16} repeats={3} duration={60} ariaLabel="Socios comerciales" class="py-8">
      {#each items as item, i}
        ...
        {:else}
          <span class="text-white/30 text-base md:text-lg font-semibold tracking-wide">{item.name}</span>
        {/if}
